...
from denseflow.transforms import UniformDequantization, VariationalDequantization, ScalarAffineBijection, Squeeze2d, \
    Conv1x1, Slice, SimpleMaxPoolSurjection2d, ActNormBijection2d, WaveletSqueeze2d
...
class DenseFlow(Flow):
    ...
    def __init__(self, data_shape=(3, 32, 32), block_config=[2, 4, 3], layers_config=[2, 2, 2],
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:  # split operation
        ...
        transforms = [
            VariationalDequantization(encoder=DequantizationFlow(data_shape, num_bits=num_bits), num_bits=num_bits,
                                      coef=coef), *transforms]
