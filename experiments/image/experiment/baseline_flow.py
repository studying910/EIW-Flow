...
def add_exp_args(parser):
    ...
    parser.add_argument('--resume', type=str, default=None)  # 'DF_74_10'
    ...
    def train_fn(self, epoch):
        ...
        for x in self.train_loader:
            ...
            print('Training. Epoch: {}/{}, Datapoint: {}/{}, Bits/dim: {:.3f} Grad Norm: {:.3f}'.format(epoch + 1,
                                                                                                        self.args.epochs,
                                                                                                        loss_count, len(
                    self.train_loader.dataset), loss_sum / loss_count, grad_norm.item()), end='\r')  # back to the head
        print('')
    ...
    def eval_fn(self, epoch):
        ...
        with torch.no_grad():
            ...
            for x in self.eval_loader:
                ...
                loss_sum += loss.detach().cpu().item() * len(x)
                loss_count += len(x)
                print('Evaluating. Epoch: {}/{}, Datapoint: {}/{}, Bits/dim: {:.3f}'.format(epoch + 1, self.args.epochs,
                                                                                            loss_count,
            ...
            samples = samples / 255.
            save_image(samples, fp=f"./generated_samples_baseline_flow_{self.args.dataset}/im{epoch}.png", nrow=8)
        return {'bpd': loss_sum / loss_count}
