...
class FlowExperiment(BaseExperiment):
    ...
    def train_fn(self, epoch):
        ...
        for x in self.train_loader:
            ...
            loss, scale_loss = elbo_bpd(self.model, x.to(self.args.device))
            total_loss = loss + self.args.sigma * scale_loss
            total_loss.backward()
        ...
        del x, loss, scale_loss, total_loss, t0, t1
    ...
    def eval_fn(self, epoch):
        ...
        with torch.no_grad():
            ...
            for x in self.eval_loader:
                loss = elbo_bpd(self.model, x.to(self.args.device))[0]
                loss_sum += loss.detach().cpu().item() * len(x)
                loss_count += len(x)
            ...
            print('')
            samples = self.model.sample(256).cpu().float()
            samples = samples / 255.
            path_train_samples = '{}/train_samples/epoch{}.png'.format(self.log_path, epoch+1)
            if not os.path.exists(os.path.dirname(path_train_samples)):
                os.mkdir(os.path.dirname(path_train_samples))
            save_image(samples, fp=path_train_samples, nrow=16)
        return {'bpd': loss_sum/loss_count}