...
class NystromAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # else:
        q_landmarks = q.reshape(N, self.num_landmarks, (H * W) // self.num_landmarks, self.embed_dim).mean(dim=-2).div_(
            (H * W) ** .5)
        k_landmarks = k.reshape(N, self.num_landmarks, (H * W) // self.num_landmarks, self.embed_dim).mean(dim=-2).div_(
            (H * W) ** .5)
