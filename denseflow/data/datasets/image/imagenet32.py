...
class ImageNet32Dataset(data.Dataset):
    ...
    urls = [
        'https://image-net.org/data/downsample/Imagenet32_train.zip'
        'https://image-net.org/data/downsample/Imagenet32_val.zip'
    ]
    ...
    def __init__(self, root=DATA_PATH, train=True, transform=None, download=False):
        ...
        if self.train:
            self.files = [os.path.join(self.processed_train_folder, file) for file in
                          os.listdir(self.processed_train_folder)]
        else:
            self.files = [os.path.join(self.processed_valid_folder, file) for file in
                          os.listdir(self.processed_valid_folder)]
    ...
    def process(self):
        ...
        with zipfile.ZipFile(self.raw_file_paths[0], "r") as zip_ref:
            zip_ref.extractall(self.processed_train_folder)

        print("Extracting validation data...")
        # tar = tarfile.open(self.raw_file_paths[1])
        # tar.extractall(self.processed_data_folder)
        # tar.close()
        with zipfile.ZipFile(self.raw_file_paths[1], "r") as zip_ref:
            zip_ref.extractall(self.processed_valid_folder)
